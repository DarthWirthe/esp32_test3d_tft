...
class Mesh
{
  public:
	...
	void drawTriangles(Engine3D &e, Color color = -1, triangleShader ts = 0)
	{
		if(ts == 0)
		{
			if(tTriNormals)
	...
	void drawEdges(TFT_eSprite &tft, Color color)
	{
		for (int i = 0; i < edgeCount; i++)
		{
			tft.drawLine(tvertices[edges[i][0]][0], tvertices[edges[i][0]][1], tvertices[edges[i][1]][0], tvertices[edges[i][1]][1], color);
		}
	}

	void drawVertices(TFT_eSprite &tft, Color color)
	{
		for (int i = 0; i < vertexCount; i++)
			tft.drawPixel(tvertices[i][0], tvertices[i][1], color);
	}
